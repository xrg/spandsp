 ...
 *
 * $Id: v42.c,v 1.5 2005/01/12 13:39:25 steveu Exp $
 */
...
static int lapm_tx_frame(lapm_state_t *s, uint8_t *frame, int len)
{
    ...
    if ((s->debug & LAPM_DEBUG_LAPM_DUMP))
        lapm_dump(frame, len, s->debug & LAPM_DEBUG_LAPM_RAW, TRUE);
    /*endif*/
...
static void lapm_send_ua(lapm_state_t *s, int pfbit)
{
    ...
    if ((s->debug & LAPM_DEBUG_LAPM_STATE))
        fprintf(stderr, "Sending unnumbered acknowledgement\n");
    /*endif*/
...
static void lapm_send_sabme(sp_sched_state_t *ss, void *user_data)
{
    ...
    frame[1] = 0x7F;
    if ((s->debug & LAPM_DEBUG_LAPM_STATE))
        fprintf(stderr, "Sending Set Asynchronous Balanced Mode Extended\n");
    /*endif*/
    lapm_tx_frame(s, frame, 3);
...
static void lapm_ack_rx(lapm_state_t *s, int ack)
{
        ...
        (s->last_frame_peer_acknowledged > s->next_tx_frame  &&  (ack > s->last_frame_peer_acknowledged  ||  ack < s->next_tx_frame)))
    {
        ...
        fprintf(stderr, "ACK received outside window, ignoring\n");
        return;
    }
    /*endif*/
    ...
    {
        if ((s->debug & LAPM_DEBUG_LAPM_STATE))
            fprintf(stderr, "-- Since there was nothing left, stopping timer T_401\n");
        /*endif*/
    ...
    {
        if ((s->debug & LAPM_DEBUG_LAPM_STATE))
            fprintf(stderr, "-- Stopping timer T_403, since we got an ACK\n");
        /*endif*/
    ...
    {
        if ((s->debug & LAPM_DEBUG_LAPM_STATE))
            fprintf(stderr, "-- Nothing left, starting timer T_403\n");
        /*endif*/
...
static void lapm_reject(lapm_state_t *s)
{
    ...
    if ((s->debug & LAPM_DEBUG_LAPM_STATE))
        fprintf(stderr, "Sending REJ (reject (%d)\n", s->next_expected_frame);
    /*endif*/
...
static void lapm_rr(lapm_state_t *s, int pfbit)
{
    ...
    if ((s->debug & LAPM_DEBUG_LAPM_STATE))
        fprintf(stderr, "Sending RR (receiver ready) (%d)\n", s->next_expected_frame);
    /*endif*/
    lapm_tx_frame(s, frame, 4);
}
/*- End of function --------------------------------------------------------*/
...
static void t401_expired(sp_sched_state_t *ss, void *user_data)
{
    ...
    if (s->txqueue)
    {
        ...
        if ((s->debug & LAPM_DEBUG_LAPM_STATE))
            fprintf(stderr, "-- Timer T_401 expired, What to do...\n");
        /*endif*/
        ...
        if (++s->retransmissions <= N_400)
        {
            ...
            if ((s->debug & LAPM_DEBUG_LAPM_STATE))
                fprintf(stderr, "-- Retransmitting %d bytes\n", s->txqueue->len);
            /*endif*/
    ...
    {
        fprintf(stderr, "Timer T_401 expired. Nothing to send...\n");
        s->t401_timer = -1;
    }
    /*endif*/
}
...
int lapm_tx_iframe(lapm_state_t *s, const void *buf, int len, int cr)
{
    ...
    {
        if ((s->debug & LAPM_DEBUG_LAPM_STATE))
            fprintf(stderr, "Stopping T_403 timer\n");
        /*endif*/
        sp_schedule_del(&s->sched, s->t403_timer);
        s->t403_timer = -1;
    }
    ...
    {
        if ((s->debug & LAPM_DEBUG_LAPM_STATE))
            fprintf(stderr, "Timer T_401 already running (%d)\n", s->t401_timer);
        /*endif*/
    }
    else
    {
        if ((s->debug & LAPM_DEBUG_LAPM_STATE))
            fprintf(stderr, "Starting timer T_401\n");
        /*endif*/
        s->t401_timer = sp_schedule_event(&s->sched, T_401, t401_expired, s);
    }
    /*endif*/
...
static void t403_expired(sp_sched_state_t *ss, void *user_data)
{
    ...
    s = (lapm_state_t *) user_data;
    if ((s->debug & LAPM_DEBUG_LAPM_STATE))
        fprintf(stderr, "Timer T_403 expired. Sending RR and scheduling T_403 again\n");
    /*endif*/
    /* Solicit an F-bit in the other end's RR */
    s->solicit_f_bit = TRUE;
    lapm_rr(s, 1);
    /* Restart ourselves */
printf("T403 expired at %lld\n", sp_schedule_time(&s->sched));
    s->t403_timer = sp_schedule_event(&s->sched, T_403, t403_expired, s);
}
/*- End of function --------------------------------------------------------*/
...
void lapm_receive(void *user_data, int ok, const uint8_t *frame, int len)
{
    ...
    if ((s->debug & LAPM_DEBUG_LAPM_DUMP))
        lapm_dump(frame, len, s->debug & LAPM_DEBUG_LAPM_RAW, FALSE);
    /*endif*/
    octet = 0;
    ...
    case LAPM_FRAMETYPE_I_ALT:
        ...
        {
            fprintf(stderr, "!! Got an I-frame while link state is %d\n", s->state);
            break;
        }
        /*endif*/
        /* Information frame */
        if (len < 4)
        {
            fprintf(stderr, "!! Received short I-frame (expected 4, got %d)\n", len);
            break;
        }
        /*endif*/
    ...
    case LAPM_FRAMETYPE_S:
        ...
        if (len < 4)
        {
            fprintf(stderr, "!! Received short S-frame (expected 4, got %d)\n", len);
            break;
        }
        /*endif*/
        ...
        case 0x00:
            ...
            if ((frame[2] & 0x01))
            {
                ...
                {
                    if ((s->debug & LAPM_DEBUG_LAPM_STATE))
                        fprintf(stderr, "-- Got RR response to our frame\n");
                    /*endif*/
                }
                else
                {
                    if ((s->debug & LAPM_DEBUG_LAPM_STATE))
                        fprintf(stderr, "-- Unsolicited RR with P/F bit, responding\n");
                    /*endif*/
                    lapm_rr(s, 1);
                }
                /*endif*/
        ...
        case 0x04:
            ...
            if ((s->debug & LAPM_DEBUG_LAPM_STATE))
                fprintf(stderr, "-- Got receiver not ready\n");
            /*endif*/
            s->busy = TRUE;
            break;   
        case 0x08:
            ...
            /* Just retransmit */
            if ((s->debug & LAPM_DEBUG_LAPM_STATE))
                fprintf(stderr, "-- Got reject requesting packet %d...  Retransmitting.\n", frame[2] >> 1);
            /*endif*/
            if ((frame[2] & 0x01))
            {
            ...
            if (!sendnow)
            {
                ...
                else
                {
                    ...
                    fprintf(stderr, "!! Got reject for frame %d, but we have nothing -- resetting!\n", frame[2] >> 1);
                    s->last_frame_peer_acknowledged =
    ...
    case LAPM_FRAMETYPE_U:
        ...
        case 0x00:
            ...
            fprintf(stderr, "UI (unnumbered information) not implemented.\n");
            break;
        case 0x0C:
            /* DM (disconnect mode) */
            if ((frame[octet] & 0x10))
            {
                if ((s->debug & LAPM_DEBUG_LAPM_STATE))
                    fprintf(stderr, "-- Got Unconnected Mode from peer.\n");
                /*endif*/
            ...
            {
                if ((s->debug & LAPM_DEBUG_LAPM_STATE))
                    fprintf(stderr, "-- DM (disconnect mode) requesting SABME, starting.\n");
                /*endif*/
        ...
        case 0x40:
            /* DISC (disconnect) */
            if ((s->debug & LAPM_DEBUG_LAPM_STATE))
                fprintf(stderr, "-- Got DISC (disconnect) from peer.\n");
            /*endif*/
            /* Acknowledge */
        ...
        case 0x60:
            ...
            {
                if ((s->debug & LAPM_DEBUG_LAPM_STATE))
                    fprintf(stderr, "-- Got UA (unnumbered acknowledgement) from %s peer. Link up.\n", (frame[0] & 0x02)  ?  "cpe"  :  "network");
                /*endif*/
                lapm_link_up(s);
            }
            else
            {
                fprintf(stderr, "!! Got a UA (unnumbered acknowledgement) in state %d\n", s->state);
            }
            /*endif*/
            break;
        case 0x6C:
            ...
            if ((s->debug & LAPM_DEBUG_LAPM_STATE))
                fprintf(stderr, "-- Got SABME (set asynchronous balanced mode extended) from %s peer.\n", (frame[0] & 0x02)  ?  "network"  :  "cpe");
            /*endif*/
            if ((frame[0] & 0x02))
            {
                ...
                if (s->we_are_originator)
                {
                    ...
                    fprintf(stderr, "We think we are the originator, but they think so too.");
                    break;
                }
                /*endif*/
            }
            else
            {
                ...
                if (!s->we_are_originator)
                {
                    ...
                    fprintf(stderr, "We think we are the answerer, but they think so too.\n");
                    break;
                }
                /*endif*/
            }
        ...
        case 0x84:
            /* FRMR (frame reject) */
            fprintf(stderr, "!! FRMR (frame reject).\n");
            break;
        case 0xAC:
            /* XID (exchange identification) */
            fprintf(stderr, "!! XID (exchange identification) frames not supported\n");
            break;
        case 0xE0:
            /* TEST (test) */
            fprintf(stderr, "!! TEST (test) frames not supported\n");
            break;
        default:
            fprintf(stderr, "!! Don't know what to do with M=%X u-frames\n", m_field);
            break;
        }
        /*endswitch*/
        break;
...
void lapm_restart(lapm_state_t *s)
{
    ...
    {
        fprintf(stderr, "!! lapm_restart: Not in 'Link Connection Released' state\n");
        return;
    }
    /*endif*/
#endif
